...
void TaskInputDialog::createWidgets() {
    ...
    taskNameEdit->setValidator(
        new QRegExpValidator(QRegExp("[A-Za-z\\d\\s_]+"), this));
...
void TaskInputDialog::acceptInput() {
    QString deadline = taskDeadlineDateEdit->text() + " " +
                       taskDeadlineTimeEdit->text();
    emit accepted(taskNameEdit->text(), taskDescEdit->text(),
                  deadline, remainderBox->currentText());
}

void TaskInputDialog::setFields(const QString &taskName, const QString &taskDesc,
